...
const DefaultIconCalendar = (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true" className="size-6 text-indigo-600 dark:text-white">
...
const Subscribe: FC<SubscribeProps> = ({
  ...
  return (
    <section id={id} className={`relative isolate overflow-hidden bg-white dark:bg-gray-900 py-16 sm:py-24 lg:py-32 ${className}`}>
          ...
          <div className="max-w-xl lg:max-w-lg">
            ...
            <form className="mt-6 flex max-w-md gap-x-4" action="#" method="post">
              ...
              <input
                ...
                autoComplete="email"
                required
                className="min-w-0 flex-auto rounded-md bg-white px-3.5 py-2 text-base text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-500 sm:text-sm dark:bg-white/5 dark:text-white dark:placeholder:text-gray-500 dark:ring-1 dark:ring-inset dark:ring-white/10 dark:focus:ring-2 dark:focus:ring-indigo-500"
              />
              <Button type="submit" variant="primary" size="md" aria-label="Подписаться">
                Подписаться
              </Button>
            </form>
          </div>
          ...
          <dl className="grid grid-cols-1 gap-x-8 gap-y-10 sm:grid-cols-2 lg:pt-2">
            {features.map((f, idx) => (
              <div className="flex flex-col items-start" key={idx}>
                <div className="rounded-md bg-indigo-50 p-2 ring-1 ring-indigo-100 dark:bg-white/5 dark:ring-white/10">
      ...
      <div aria-hidden="true" className="absolute left-1/2 top-0 -z-10 -translate-x-1/2 blur-3xl xl:-top-6">
        <div
          className="h-64 w-[36rem] bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30"
